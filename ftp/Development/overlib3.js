...
//\
//\  Modified on 2001-09-07 by Robert E. Boughner to add a mouseOut feature
//\  to the overlib popup and to bring it in to conformance to previous
//\  modifications made to version 3.33.  These changes can be found by
//\  searching for the string "Added REB".
//\
//\     Corrected an error with IE5/Mac by making sure unit specifier is
//\     added when setting left/top values of an object.
//\
//\     Added code to cancel any previous timeout event if the popup
//\     is STICKY.
//\
...
// Our layer
var over = null;
var hSwitch;    // Added REB for mouseOut feature in IE and NS6

// Decide browser version
//var ns4 = (document.layers)? true:false;
var ns4 = (navigator.appName == 'Netscape' && parseInt(navigator.appVersion) == 4);  // Added REB more positive check
var ns6 = (document.getElementById)? true:false;
var ie4 = (document.all)? true:false;
var ie5 = false;
// Added REB -- Resize fix for NS4.x to keep track of layer
if (ns4) {
...
function overlib() {
	...
	// Added REB -- Set mouseover switch
	hSwitch = false;
...
// Added REB -- Cancel any previous settings of mouseouts or mouseovers
	var Obj = (ie4) ? o3_frame.document.all['overDiv'] : over;
   if (ie4 || ns6) Obj.onmouseover = null;
   if (ns4) Obj.onmouseout = null;
	...
	for (i = 0; i < ar.length; i++) {
		...
		if (parsemode < 0) {
			...
			if (ar[i] == NOCLOSE) { opt_NOCLOSE(ar[i]); continue; }  // Added REB
...
function overlib350() {
	...
	// We want it to stick!
	if (o3_sticky) {
		if (o3_timerid > 0) {  // Added REB -- cancel any timeout event that was posted
...
function placeLayer() {
	...
	if (o3_fixx > -1) {
		...
		if (ns4 || ns6) iwidth = o3_frame.innerWidth; // Added REB
		...
		if (o3_hpos == CENTER) { // Center
			...
			if (placeX < winLeft) placeX = winLeft;  // Added REB
		}
	...
	if (o3_fixy > -1) {
		...
		if (o3_vauto == 1) {
			...
			if (ns4 || ns6) iheight = o3_frame.innerHeight;  // Added REB
...
// Added REB -- for mouseOut handler
...
function layerWrite(txt) {
	txt += "\n";
		...
		range.setStartAfter(over);  // Added REB
...
function repositionTo(obj,xL,yL) {
	if ( (ns4) || (ie4) ) {
	        obj.left = (ie4 ? xL + 'px' : xL);  // Added REB for IE5/Mac problems
	        obj.top = (ie4 ? yL + 'px' : yL);   // Added REB for IE5/Mac problems
	} else if (ns6) {
...
// Added REB -- Check position of cursor relative to overDiv DIVision; mouseOut function
function cursorOff() {
   var Obj = (ie4) ? o3_frame.document.all['overDiv'] : over;
...
// Added REB -- Sets use of close text and set mouseOut functions
function opt_NOCLOSE(unused) {
	...
	if (ie4 || ns6) Obj.onmouseover = function () {hSwitch = true;}
	return 0;
}
